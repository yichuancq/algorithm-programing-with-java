package com.example.algorithm.matrixgraph;

import com.example.algorithm.matrix.Matrix;
import com.example.algorithm.queue.LinkedQueue;
import com.example.algorithm.seqlist.SeqList;
...
//    System.out.print(pathlink.toString()+"长度"+(dist[j]==MAX_WEIGHT ? "∞" : dist[j])+"，");
    ...
    {
        SinglyList<T> pathlink = new SinglyList<T>();//路径单链表，记录最短路径经过的各顶点，用于反序
        ((SinglyList<T>) pathlink).insert(0, this.getVertex(j));     //单链表插入最短路径终点vj

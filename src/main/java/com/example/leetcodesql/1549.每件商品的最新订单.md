...
```roomsql
select 
    b.product_name, a.product_id, a.order_id, a.order_date
from
    (select *, dense_rank() over (partition by product_id order by order_date desc) as ran
    from Orders) a 
join 
    products b 
on 
...
where 
    ran =   1
order by 
    b.product_name, a.product_id, a.order_id
;
